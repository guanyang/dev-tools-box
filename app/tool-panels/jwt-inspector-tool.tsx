...
export default function JwtInspectorTool() {
  ...
  function inspect() {
    ...
    catch { setInspection(null); setError(token.trim() ? "JWT 内容无法解析，请确认它是由两个点分隔的三段式 Token。" : "请输入需要解析的 JWT Token。"); }
  }
  ...
  async function verify() {
    try {
      ...
      setError("");
    } catch { setVerification(""); setError(token.trim() ? "签名校验失败，请检查 JWT 内容、算法与 JWK 是否匹配。" : "请输入需要校验的 JWT Token。"); }
  }
  ...
  return <section className="tool-panel">
    ...
    {verification && <div role={verification.startsWith("签名有效") ? "status" : "alert"} className={verification.startsWith("签名有效") ? "success-banner" : "error-banner"}>{verification}</div>}
